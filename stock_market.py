...
def statement_generator(actions, stock_actions):
    #check date on first action and stock_action (use to generate base day_message)
        #get appropriate message(helper) and add to day_message
        #if both the same add action then stock_action to day_message
    #move counter for list
    #print day_message or put into a final list and clear day
    #repeat until lists are both empty
    
def generate_day_message(day):
    return "On " + day + ", you have:\n"
    
def generate_action_statement(entry):
    return statement
    
def generate_stock_statement(entry):
